...
import { AsrContextFiles, validateHotwords } from '../../src/ai/asr-context-files.js'
...
test('reloads hotword and background files on every read', (t) => {
  ...
  try {
    ...
    const files = new AsrContextFiles(hotwordsPath, backgroundPath)

    assert.equal(files.readHotwords(), undefined)
    assert.equal(files.readBackground(), undefined)

    writeFileSync(hotwordsPath, JSON.stringify({ CQ: 5 }))
    writeFileSync(backgroundPath, '背景文字')
    assert.deepEqual(files.readHotwords(), { CQ: 5 })
    assert.equal(files.readBackground(), '背景文字')

    writeFileSync(hotwordsPath, JSON.stringify({ 签到: 4 }))
    assert.deepEqual(files.readHotwords(), { 签到: 4 })
    ...
    assert.deepEqual(files.readHotwords(), { 呼叫: 3 })
...
test('truncates background text at the context limit', (t) => {
  ...
  try {
    const backgroundPath = join(dir, 'background.txt')
    const files = new AsrContextFiles(join(dir, 'missing.json'), backgroundPath)
    writeFileSync(backgroundPath, '字'.repeat(450))
    const background = files.readBackground()
