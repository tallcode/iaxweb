...
export function createConnectionOptions(config: AppConfig['nats']): ConnectionOptions {
  return {
...
interface NatsAudioServiceOptions {
  config: AppConfig['nats']
  nodeId: string
  onAudio: (data: Uint8Array) => void
  onEvent: (text: string) => void
  onState: (state: AudioStateEvent) => void
  retryIntervalMs?: number
}

const OFFLINE_AUDIO_STATE: AudioStateEvent = { type: 'state', online: false, speaking: false }
const LISTENER_HEARTBEAT_MS = 15_000

export class NatsAudioService {
  private readonly config: AppConfig['nats']
  private readonly onAudio: (data: Uint8Array) => void
  private readonly onEvent: (text: string) => void
  private readonly onState: (state: AudioStateEvent) => void
  ...
  private state: AudioStateEvent = OFFLINE_AUDIO_STATE
  private readonly gatewayId = randomUUID()
  private listenerCount = 0

  constructor(options: NatsAudioServiceOptions) {
  ...
  get currentState(): AudioStateEvent {
    return this.state
  }
  ...
  start(): void {
    if (!this.stopped)
      return
    this.stopped = false
    this.heartbeatTimer = setInterval(() => this.publishListeners(), LISTENER_HEARTBEAT_MS)
  ...
  private async watchConnection(connection: NatsConnection): Promise<void> {
    for await (const status of connection.status()) {
      ...
      if (status.type === 'disconnect') {
        this.transportConnected = false
        this.setState(OFFLINE_AUDIO_STATE)
      }
  ...
  private setState(state: AudioStateEvent): void {
    this.state = state
    this.onState(state)
  }
...
export function isAudioStateEvent(value: unknown): value is AudioStateEvent {
